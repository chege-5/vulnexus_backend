...
class GreyNoiseProvider(SimpleJsonProvider):
    ...
    async def lookup(self, client: httpx.AsyncClient, query: str, *, context: dict[str, Any] | None = None, limit: int = 5):
        ...
        response = await client.get(f"{self.settings.endpoint.rstrip('/')}/community/{query}", headers={"key": self.settings.api_key, "accept": "application/json"})
        ...
        items = [{"identifier": query, "summary": payload.get("classification") or payload.get("message"), "vendor": "GreyNoise", "raw": payload}]
        return normalize_results(self.provider_name, query, items, raw=payload, status_code=response.status_code)


register_simple_provider("greynoise", lambda settings_: GreyNoiseProvider(settings_))
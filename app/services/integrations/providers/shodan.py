...
class ShodanProvider(SimpleJsonProvider):
    ...
    async def lookup(self, client: httpx.AsyncClient, query: str, *, context: dict[str, Any] | None = None, limit: int = 5):
        ...
        response = await client.get(
            f"{self.settings.endpoint.rstrip('/')}/shodan/host/{query}",
        ...
        items = [{
            "identifier": query,
            "summary": payload.get("org") or payload.get("isp") or payload.get("data", [{}])[0].get("product") if payload.get("data") else None,
            "vendor": "Shodan",
            "raw": payload,
        }]
